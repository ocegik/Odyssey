...
export default function MockLogTab({
  ...
  const addBlock = (sectionIdx, type) => {
    setMockForm((form) => ({
      ...
      sections: form.sections.map((section, idx) => {
        ...
        const newBlocks = [...section.questionBlocks, newBlock];
        return { ...section, questionBlocks: renumberBlockNames(newBlocks, newBlock.id) };
      }),
    }));
  };
  ...
  const submitMock = (ev) => {
    ...
    const payload = mockFormToPayload(mockForm);
    if (editingMockId) {
