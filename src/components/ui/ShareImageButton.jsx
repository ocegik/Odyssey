...
export default function ShareImageButton({ createImage, label = "Share" }) {
  ...
  return <>
    ...
    {preview && <div role="dialog" aria-modal="true" aria-label="Share image preview" className="fixed inset-0 z-50 flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.72)" }}>
      <div className="w-full max-w-4xl p-4 sm:p-5" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 14, boxShadow: "var(--shadow-floating)" }}>
        <div className="mb-4 flex items-center justify-between gap-3"><div><h2 style={{ color: COLORS.ink, fontFamily: "'Space Grotesk', sans-serif", fontSize: 17, fontWeight: 700 }}>Preview</h2><p className="text-xs mt-0.5" style={{ color: COLORS.inkMuted }}>This exact image will be downloaded.</p></div><button type="button" aria-label="Close preview" onClick={close} className="theme-hover p-2" style={{ background: "transparent", border: `1px solid ${COLORS.border}`, borderRadius: 7, color: COLORS.inkMuted }}><X size={16} /></button></div>
        <img src={preview.url} alt="Share image preview" className="w-full" style={{ borderRadius: 9, border: `1px solid ${COLORS.border}` }} />
        <div className="mt-4 flex justify-end gap-2"><button type="button" onClick={close} className="theme-hover px-3 py-2 text-sm" style={{ background: "transparent", border: `1px solid ${COLORS.border}`, borderRadius: 8, color: COLORS.ink }}>Close</button><button type="button" onClick={download} className="inline-flex items-center gap-2 px-3 py-2 text-sm" style={{ background: COLORS.primary, border: "none", borderRadius: 8, color: COLORS.onPrimary, fontWeight: 650 }}><Download size={15} /> Download PNG</button></div>
      </div>
    </div>}
  </>;
}
