...
function SegmentedControl({ options, value, onChange }) {
  return (
    <div className="flex gap-1 p-1 flex-wrap" style={{ background: COLORS.surface2, border: `1px solid ${COLORS.border}`, borderRadius: 9 }}>
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={`px-2.5 py-1.5 text-xs ${active ? "" : "hover:bg-black/5"}`}
            style={{
    ...
    <div className="flex flex-col gap-3 p-4" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12 }}>
      <div className="flex items-center gap-2">
        ...
        <button
          type="button"
          onClick={onExpandAll}
          className="inline-flex items-center gap-1.5 px-2.5 py-2 text-xs hover:bg-black/[0.04] shrink-0"
